...
string Tile::pack() {
    ...
    packedString += title;
    packedString += "::";
    packedString += description;
    packedString += "::";
    
    // Packing possibleEnemies vector size
    packedString += to_string(possibleEnemies.size());
    packedString += "::";
    
    // Packing possibleEnemies vector cells
    for (int i = 0; i < possibleEnemies.size(); i++) {
        packedString += to_string(possibleEnemies[i]);
        packedString += "::";
    }
    
    packedString += to_string(staticLoot);
    packedString += "::";
    
    // Packing possibleLoot vector size
    packedString += to_string(possibleLoot.size());
    packedString += "::";
    
    // Packing possibleLoot vector cells
    for (int i = 0; i < possibleLoot.size(); i++) {
        packedString += to_string(possibleLoot[i]);
        packedString += "::";
    }
    
    packedString += to_string(tileType);
    packedString += "::";
    
    // Packing possibleDirections vector size
    packedString += to_string(possibleDirections.size());
    packedString += "::";
    
    // Packing possibleDirections vector cells
    for (int i = 0; i < possibleDirections.size(); i++) {
        packedString += to_string(possibleDirections[i]);
        packedString += "::";
    }
    
    packedString += to_string(climateType);
    packedString += "::";
    
    // Packing climateDesc vector size
    packedString += to_string(climateDesc.size());
    packedString += "::";
    
    // Packing climateDesc vector cells
    for (int i = 0; i < climateDesc.size(); i++) {
        packedString += to_string(climateDesc.size());
        packedString += "::";
    }
    
    // Packing x & y Coordinates
    packedString += x;
    packedString += "::";
    packedString += y;
...
void Tile::unpack(string packedString) {
    string token;
    for (int i = 0; i < packedString.size(); i++) {
    }
}
